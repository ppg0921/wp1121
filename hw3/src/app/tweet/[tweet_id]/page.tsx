...
import { db } from "@/db";
import { likesTable, tweetsTable, usersTable, commentsTable } from "@/db/schema";
...
export default async function TweetPage({
  ...
  const [liked] = await db
    ...
    .where(
      ...
      // the user clicked like to the tweet
    )
    .execute();
  // get the user who tweeted the activity
  const [user] = await db
    .select({
  ...
  const replies = await db
    ...
    .from(commentsTable)
    .where(eq(commentsTable.replyToTweetId, tweet_id_num))
    .orderBy(desc(commentsTable.createdAt))
    .innerJoin(usersTable, eq(commentsTable.userHandle, usersTable.handle))
    // .leftJoin(likesSubquery, eq(commentsTable.id, likesSubquery.tweetId))
    // .leftJoin(likedSubquery, eq(commentsTable.id, likedSubquery.tweetId))
    .execute();
    ...
    <>
      <div className="flex h-screen w-full max-w-2xl flex-col overflow-scroll pt-2">
        ...
        <div className="flex flex-col px-4 pt-3">
          ...
          <div className="flex">
            <div className="flex-col the gray part">
              <div>
                <article className="mt-3 whitespace-pre-wrap text-xl">
                  {tweet.content}
                </article>
                <div> {tweet.likes} {tweet.likes > 1 ? "people are" : "person is"} participating</div>
              </div>
              <div>
                <time className="my-4 block text-sm text-gray-500">
                  <TimeText date={tweet.createdAt} format="h:mm A · D MMM YYYY" />
                </time>
                <div> starts at {tweet.startDate}, ends at {tweet.endDate}</div>
                <Separator />
              ...
              <LikeButton
                ...
                tweetId={tweet.id}
              />
              {/* <Button>{tweet.liked?"I want to quit":"I want to join"}</Button> */}
          </div>
        </div>
