  ...
  return (
    <>
      <div className="flex h-screen w-full max-w-2xl flex-col overflow-scroll pt-2">
        <h1 className="mb-2 bg-white px-4 text-4xl">{username}</h1>
        <div className="flex w-full px-4 pt-3">
          <div className="w-full px-6 pt-3 space-x-8">
            <NameButton handle={handle} />
            {/* <TweetInput/> */}
            <NewActivityButton handle={handle} />
          </div>
        </div>
        <Separator />
        <ActivityList tweets={tweets} searchParams={searchParamsToList} />
        {/* {tweets.map((tweet) => (
          <Tweet
            key={tweet.id}
            id={tweet.id}
