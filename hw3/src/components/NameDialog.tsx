...
export default function NameDialog({openFromOutSide, onClose}:{openFromOutSide:boolean, onClose:()=>void}) {
  ...
  const handleSave = () => {
    ...
    if (newUsernameError || newHandleError) {
      if(!openFromOutSide)
        return false;
      if(newUsernameError){
        alert("the new username is not valid");
        onClose();
        // console.log("here3");
        setDialogOpen(false);
        return false;
    ...
    if(handle)
      ...
      params.set("handle", searchParams.get("handle")??"fakeHandle");
  ...
  return (
    <Dialog open={openFromOutSide||dialogOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          {!openFromOutSide && <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="name" className="text-right">
              Handle
            </Label>
