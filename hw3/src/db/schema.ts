...
export const tweetsTable = pgTable(
  "tweets",
  {
    ...
    startDate: varchar("start_date", { length: 15 }).notNull(),
    endDate: varchar("end_date", { length: 15 }).notNull(),
...
export const participationTable = pgTable(
  "participation",
  {
    ...
    userHandle: varchar("user_handle", { length: 50 })
      .notNull()
      .references(() => usersTable.handle, { onDelete: "cascade" }),
    participatingTweetId: integer("participating_tweet_id")
      .notNull()
      .references(() => tweetsTable.id, { onDelete: "cascade" }),
    relativePosition: integer("relative_position"),
  },
  (table) => ({
    participatingTweetIdIndex: index("participating_tweet_id_index").on(table.participatingTweetId),
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    // unique constraints ensure that there are no duplicate combinations of
    // values in the table. In this case, we want to ensure that a user can't
    // like the same tweet twice.
    uniqCombination: unique().on(table.userHandle, table.participatingTweetId),
  }),
);



// export const timeSlotTable = pgTable(
//   "timeSlot",
//   {
//     id: serial("id").primaryKey(),
//     representingTweetId: integer("representing_tweet_id"),
//     relativePosition: integer("relative_position"),
//     participantHandles: varchar("user_handle", { length: 50 }).notNull()
//       .references(() => usersTable.handle, {
//         onDelete: "cascade",
//         onUpdate: "cascade",
//       }).array(),
//   },
//   (table) => ({
//     tweetHandleIndex: index("tweet_id_index").on(table.representingTweetId),
//     timeIdIndex: index("time_id_index").on(table.id),
//     // we can even set composite indexes, which are indexes on multiple columns
//     // learn more about composite indexes here:
//     // https://planetscale.com/learn/courses/mysql-for-developers/indexes/composite-indexes
//   }),
...
//   },
//   (table) => ({
//     representingTweetIndex: index("representing_tweet_index").on(table.representingTweetId),
//     // createdAtIndex: index("created_at_index").on(table.createdAt),
//   }),
// );